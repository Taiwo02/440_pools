...
**Start a conversation as a non-logged-in visitor**  
No auth required. Rate-limited to ~3 requests / minute per IP.

> If this visitor already has an active (non-closed) conversation (matched by email/phone), the server returns that existing conversation and a fresh token.
...
|-------|------|----------|-------|
| `firstName` | string (1–50) | Yes | |
| `lastName` | string (1–50) | Yes | |
...
Store the `token` in `sessionStorage` and pass it as `Authorization: Bearer <token>` on all subsequent requests for this conversation, and as the socket handshake token.

---

### POST `/support/conversations`  
**Start a conversation as a logged-in buyer**  
Auth: Buyer JWT required. Rate-limited to ~3 requests / minute per IP.

> The customer's name is taken from their account — no name fields needed. If the buyer already has an active (non-closed) conversation, the existing one is returned.
...
```json
{
  ...
  "data": {
    "conversation": {
      "id": 43,
      "referenceCode": "SUP-20260413-C2A9",
...
> No `token` is returned — continue using the existing buyer JWT for all subsequent requests.
...
```js
// 1a. Start conversation (anonymous / guest)
const { data: { conversation, token } } = await api.post("/support/conversations/guest", {
  firstName: "John",
  lastName: "Doe",
  email: "user@example.com",
...
// 1b. Start conversation (logged-in buyer — use buyer JWT, no name fields needed)
const { data: { conversation } } = await api.post("/support/conversations", {
  message: "Hello, I need help",
